<?= $this->extend('mydanus/template'); ?>

<?= $this->section('maincontent'); ?>

<section id="result" class="peachgreen afternav">
    <div class="container pb-4">
        <!-- Keterangan -->
        <div class="row found pb-2">
            <h6><i class="fa-regular fa-lightbulb me-2"></i> Beberapa produk ditemukan
            </h6>
        </div>
        <div class="row notfound pb-2 hidden">
            <!-- setting class hidden => menyesuaikan hasil pencarian-->
            <h6>
                <span class="fa-stack fa-1x ms-0">
                    <i class="fa-regular fa-lightbulb fa-stack-1x"></i>
                    <i class="fa-solid fa-slash fa-stack-1x "></i>
                </span>
                Produk tidak ditemukan
            </h6>
        </div>
        <!-- Produk-produk -->
        <div class="row products">
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>

        </div>
        <div class="row products">
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card border border-warning shadow p-3">
                    <img src="img/sampel produk 1.png" class="card-img-top" alt="Gambar Kue enak">
                    <div class="card-body">
                        <p><i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-fill gold"></i>
                            <i class="bi bi-star-half gold"></i>
                        </p>
                        <h6>UKM wirus</h6>
                        <h5 class="card-title">Kue enak</h5>
                        <h4 id="price" class="pt-3">RP. 5.000</h4>
                    </div>
                </div>
            </div>

        </div>
        <!-- Pagination -->
        <div class="row pagination">
            <nav aria-label="pagination search">
                <ul class="pagination pagination-lg justify-content-center ">
                    <li class="page-item disabled "><a class="page-link">Previous</a></li>
                    <li class="page-item active"><a class="page-link" href="#">1</a></li>
                    <li class="page-item" aria-current="page"><a class="page-link" href="#">2</a></li>
                    <li class="page-item"><a class="page-link" href="#">3</a></li>
                    <li class="page-item"><a class="page-link" href="#">Next</a></li>
                </ul>
            </nav>
        </div>
    </div>
</section>

<?= $this->endSection(); ?>